...
class ImageSimilarity():
    ...
    def _sub_process(para):
        ...
        try:
            ...
            print('Error to extract feature of %s: %s' % (fields[0], e))
    ...
    def iteration(self, save_header, thresh=0, title1=None, title2=None):
        ...
        # print('title :', self._title[0], self._title[1])
        ...
        # print('fields me check :', fields1, fields2)
        ...
        for x, y in enumerate(indexes):
            dis = distances[x][y]
            if dis >= thresh:
                # print('result printing', float(dis))
                d[fields1[x][0]] = round((dis), 2)
                # print(fields1[x][0])

                result.append(np.concatenate((fields1[x], fields2[y], np.array(['%.5f' % dis])), axis=0))
        # print(d)
        aa = {k: v for k, v in sorted(d.items(), key=lambda item: item[1], reverse=True)}
        outresult = (dict(list(aa.items())[0: 10]))
...
if __name__ == '__main__':
    ...
    ap.add_argument("-i", "--input_img", required=True,
        help="path to input image")
    ap.add_argument("-d", "--Dataset_path", required=True,
        help="path to Dataset_path")
    args = vars(ap.parse_args())

    ## Output dir to save top 10 match images
    if os.path.isdir('Output_Img'):
    ...
    ## DataBase path of Img (Input from user)
    Dataset_path = args['Dataset_path'] #"/home/aadi/Downloads/visualsimilarity/bottoms_resized_png/"

    Input_Img = args['input_img'] #"/home/aadi/Downloads/visualsimilarity/bottoms_resized_png/13585938PXR.png"
    ...
    for imgname in os.listdir(Dataset_path):
        if not imgname in Input_Img.split('/')[-1]:
            imgdata = [imgname, Dataset_path + imgname]
            Dataset.append(imgdata)
 
    # '''Save features and fields'''
    similarity.save_data('test1', Dataset)
    similarity.save_data('test2', [[Input_Img.split('/')[-1], Input_Img]])

    # '''Calculate similarities'''
    result = similarity.iteration(['test1_id', 'test1_url', 'test2_id', 'test2_url'], thresh=0.0)
    
    
    ## Top most match image save in Output_Img with matched result in name of image.
    for img in result.keys():
        shutil.copy(Dataset_path + img , 'Output_Img/' + str(result[img]) + '_' + img)
